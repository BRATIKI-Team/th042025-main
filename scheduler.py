...
async def process_source_group(grouped_source, get_source_messages_usecase, notify_bot_usecase):
    """
    ...
    try:
        ...
        for bot in bots_to_notify:
            logger.info(f"Notifying bot {bot} about {len(messages)} messages from sources")
...
async def setup_scheduler():
    ...
    for period, sources in notification_periods.items():
        ...
        logger.info(f"Added job for notification period {period} minutes with {len(sources)} source groups")
...
async def process_sources_for_period(sources, get_source_messages_usecase, notify_bot_usecase):
    """
    ...
    for source in sources:
        ...
        if time_since_last_hit >= timedelta(seconds=source.notification_period):
            tasks.append(process_source_group(source, get_source_messages_usecase, notify_bot_usecase))
