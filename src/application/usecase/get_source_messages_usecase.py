...
class GetSourceMessagesUsecase:
    ...
    async def execute(self, grouped_source: GroupedSourceModel) -> List[MessageModel]:
        ...
        """
        logger.info(f"Getting messages for source group: {grouped_source.url} (type: {grouped_source.type})")
    ...
    async def _get_telegram_messages(self, grouped_source: GroupedSourceModel) -> List[MessageModel]:
        """
        ...
        try:
            ...
            existing_external_ids = await self._message_repository.exists_by_external_ids(external_ids)
            
            # Filter out messages that already exist
            new_messages = [
                msg for msg in telegram_messages 
                ...
                if msg.attachments:
                    ...
                    for attachment in msg.attachments:
                        if attachment.type == "photo":
                            ...
                            attachment_info.append(f"[Document: {attachment.file_name or 'unnamed'}]")
                        else:
